    ...
    if ($result_acct->num_rows > 0) {
        // output data of each row
                while($row = $result_acct->fetch_assoc()) {
                    $polls[] = array('id' => $row['id'],'endtime' => date('Y-m-d H:i', strtotime($row['endtime'])),'count' => $row['count'],'title' => mb_strimwidth($row['title'], 0, 32, '..'),'fulltitle' => $row['title'],'category' =>  $row['category']);
                }
